...
from finetune.datasets.utils import (
    ...
    preprocess_video_match,
    crop_padded_video,
    save_and_reload_video
)
...
class Trainer:
    ...
    def _init_distributed(self):
        ...
        ds_config = getattr(self.accelerator.state.deepspeed_plugin, "deepspeed_config", None)
        if ds_config is not None:
            if "optimizer" in ds_config:
                # TODO: only support adamw
            ...
            if "scheduler" in ds_config:
                # TODO: only supprot constant_with_warmup和decay_with_warmup
                if self.args.lr_scheduler == "constant_with_warmup":
                    ds_config["scheduler"] = {
                        "type": "WarmupLR",
                        "params": {
                            "warmup_min_lr": 0.0, # diffusers constant_with_warmup
                            "warmup_max_lr": self.args.learning_rate,
                            "warmup_type": self.args.lr_warmup_type,
                            "warmup_num_steps": self.args.lr_warmup_steps,
                        }
                    }
                    ...
                    raise ValueError(f"Unsupported scheduler: {self.args.lr_scheduler}")
            if "gradient_clipping" in ds_config:
                ds_config["gradient_clipping"] = self.args.max_grad_norm
    ...
    def prepare_dataset(self) -> None:
        ...
        elif self.args.model_type == "real-sr":
            self.dataset = RealSRDataset(
                **(self.args.model_dump()),
                device=self.accelerator.device,
                max_num_frames=self.state.train_frames,
                height=self.state.train_height,
        ...
        elif self.args.model_type == "real-sr-image-video":
            self.dataset = RealSRImageVideoDataset(
                **(self.args.model_dump()),
                device=self.accelerator.device,
                max_num_frames=self.state.train_frames,
                height=self.state.train_height,
        ...
        if self.args.is_latent or self.args.is_prompt_latent:
            ...
            logger.info("Precomputing latent for video and prompt embedding is not required")
    ...
    def prepare_optimizer(self) -> None:
        ...
        self.save_trainable_param_status()
    ...
    def prepare_for_training(self) -> None:
        if self.args.use_perceptual_loss:
            if self.args.ea_dists_weight > 0:
                self.edge_detection_model = EdgeDetectionModel().to(self.accelerator.device)
                self.dists_loss = pyiqa.create_metric('dists', device=self.accelerator.device, as_loss=True)
    ...
    def prepare_for_validation(self):
        ...
        if self.args.eval_metric_list != '':
            eval_metric_list = [m.strip().lower() for m in self.args.eval_metric_list.split(',')]
            self.eval_metric_list = {}
            for name in eval_metric_list:
                try:
                    self.eval_metric_list[name] = pyiqa.create_metric(name).to(self.accelerator.device).eval()
                except Exception as e:
                    print(f"Failed to initialize metric '{name}': {e}")
        else:
            self.eval_metric_list = None

        if self.args.validation_ref_videos is not None:
            validation_ref_videos = load_images(self.args.validation_dir / self.args.validation_ref_videos)
        else:
            validation_ref_videos = [None] * len(validation_videos)
        ...
        if self.args.validation_images is not None:
            ...
            validation_images = [None] * len(validation_videos)

        if self.args.validation_prompts is not None:
        ...
        self.state.validation_ref_videos = validation_ref_videos

    def prepare_trackers(self) -> None:
        logger.info("Initializing trackers")
        exp_name = self.args.output_dir.name
    ...
    def train(self) -> None:
        ...
        for epoch in range(first_epoch, self.args.train_epochs):
            ...
            for step, batch in enumerate(self.data_loader):
                ...
                if self.args.is_image_video_mix:
                    mode = 'image' if global_step % (self.args.image_step + self.args.video_step) < self.args.image_step else 'video'

                with accelerator.accumulate(models_to_accumulate):
                    # These weighting schemes use a uniform timestep sampling and instead post-weight the loss
                    if self.args.is_image_video_mix:
                        result = self.compute_loss(batch, mode)
                    else:
                ...
                if global_step % self.args.stastic_frequency == 0:
                    memory_statistics = get_memory_statistics()
                    logger.info(
                        f"Memory after iteration {global_step} (frequency: {self.args.stastic_frequency}): {json.dumps(memory_statistics, indent=4)}"
                    )
    ...
    def validate(self, step: int) -> None:
        ...
        for i in range(num_validation_samples):
            ...
            ref_video = self.state.validation_ref_videos[i]
            name_file = Path(video).stem
            ...
            if video is not None:
                ...
                video_pil = video.round().clamp(0, 255).to(torch.uint8)
                video_pil = [Image.fromarray(frame.permute(1, 2, 0).cpu().numpy()) for frame in video_pil]
            ...
            validation_artifacts = self.validation_step(
                {"prompt": prompt, "image": image, "video": video_pil, "video_tensor": video, "ref_video": ref_video}, pipe
            )
            
            logger.info(f'{name_file}: {prompt}')
            ...
            if isinstance(validation_artifacts[0][1], torch.Tensor):
                # For Tensor Output
                validation_artifacts_tensor = validation_artifacts[0][1].to(torch.float32)
                # [F, C, H, W] // 0, 1
                validation_artifacts_tensor = validation_artifacts_tensor.permute(1, 0, 2, 3).contiguous()
                target_f, _, target_h, target_w = ref_video.shape
                validation_artifacts_tensor = validation_artifacts_tensor[:target_f, :, :target_h, :target_w]

                _video_tensor = (validation_artifacts_tensor * 255).clamp(0, 255).to(torch.uint8)
                # Convert each frame: [C, H, W] -> [H, W, C] -> numpy -> PIL
                pil_frames = [Image.fromarray(frame.permute(1, 2, 0).cpu().numpy()) for frame in _video_tensor]
                validation_artifacts = [("video", pil_frames)]
            else:
                # For PIL List Output
                validation_artifacts = [("video", crop_padded_video(validation_artifacts[0][1], ref_video))]
                # [F, C, H, W] // 0, 1 (pyiqa 要求)
                transform = transforms.ToTensor()
                validation_artifacts_tensor = torch.stack([transform(frame) for frame in validation_artifacts[0][1]], dim=0).to(torch.float32)

            if ref_video is not None:
                ref_video = ref_video.to(torch.float32).clamp(0, 255) / 255.0
            
            if self.eval_metric_list is not None:
                results_dict = evaluate_video_metrics(
                    pred_video=validation_artifacts_tensor,
                    ref_video=ref_video,
                    models=self.eval_metric_list,
                    device=self.accelerator.device,
                    name=name_file,
                )
                all_processes_metrics.append(results_dict)
            ...
            artifacts = {
                ...
                "video": {"type": "video", "value": video_pil},
            }
            ...
            for key, value in list(artifacts.items()):
                ...
                base_path = self.args.output_dir / "validation_res" / f'Step-{step}'
                subfolder = "validation" if key.startswith("artifact_") else "origin"
                save_path = base_path / subfolder
                save_path.mkdir(parents=True, exist_ok=True)
                filename = f"{accelerator.process_index}-{name_file}-{prompt_filename}-{hash_suffix}.{extension}"
                filename = str(save_path / filename)
        ...
        all_metrics = gather_object(all_processes_metrics)

        if accelerator.is_main_process:
            if self.eval_metric_list is not None:
                metric_names = all_metrics[0].keys()
                averaged_metrics = {
                    name: round(np.mean([m[name] for m in all_metrics]), 4) for name in metric_names
                }
                metrics_logs = {}
                for k, v in averaged_metrics.items():
                    metrics_logs[k] = v
                logger.info(f"Validation metrics: {json.dumps(metrics_logs, indent=4)}")
                accelerator.log(metrics_logs, step=step)
    ...
    def save_args_and_state(self):
        ...
        save_dict = {
            ...
            "state": {k: str(v) for k, v in vars(self.state).items()},
        }
        import yaml
        with open(output_path / "args.yaml", "w", encoding="utf-8") as f:
            yaml.safe_dump(save_dict, f, sort_keys=False)

        ds_config = getattr(self.accelerator.state.deepspeed_plugin, "deepspeed_config", None)
        if ds_config is not None:
            config_path = output_path / "deepspeed_config.json"
            with open(config_path, "w", encoding="utf-8") as f:
                json.dump(ds_config, f, indent=2)

    def save_trainable_param_status(self):
        param_status = {}

        for name, param in self.components.transformer.named_parameters():
            param_status[name] = "trainable" if param.requires_grad else "frozen"

        output_path = Path(self.args.output_dir)
        output_path.mkdir(parents=True, exist_ok=True)

        save_path = output_path / "trainable_params.json"
        with open(save_path, "w", encoding="utf-8") as f:
            json.dump(param_status, f, indent=2)

        print(f"Saved parameter trainability info to {save_path}")
