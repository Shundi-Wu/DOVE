...
class RandomJPEGCompression:
    ...
    def _apply_random_compression(self, imgs):
        ...
        for img in imgs:
            ...
            # [ WARN:0@6.636] global loadsave.cpp:1329 imencode Unsupported depth image for selected encoder is fallbacked to CV_8U.
...
class RandomVideoCompression:
    ...
    def _apply_random_compression(self, imgs):
        ...
        pad_hw = (0, 0)
        for img in imgs:
            padded_img, pad_hw = pad_if_odd(img)
        ...
        with av.open(buf, 'r', 'mp4') as container:
            if container.streams.video:
                for frame in container.decode(**{'video': 0}):
                    img = frame.to_rgb().to_ndarray().astype(np.float32)
                    img = remove_padding(img, pad_hw)
                    outputs.append(img)
...
if __name__ == '__main__':
    yaml_path = 'configs/degradation.yaml'
    with open(yaml_path, 'r') as f:
        opt = yaml.safe_load(f)
    # print(yaml.dump(opt, sort_keys=False, indent=4))

    inp_path = 'dataset/VideoSR/UDM10/GT_video/000.mp4'
    ...
    out_video = out_dict['lqs']
    print(len(out_video), out_video[0].shape, out_video[0].dtype, out_video[0].max(), out_video[0].min())
    import imageio.v3 as iio
    iio.imwrite(
        'dataset/VideoSR/UDM10/Demo/000.mp4',
        [np.clip(f, 0, 255).astype(np.uint8) for f in out_video],
        fps=8,
        codec='libx264',
        pixelformat='yuv444p',
